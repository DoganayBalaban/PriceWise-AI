...
async def create_alert(
    ...
    existing = await repo.get_by_product_email(body.product_id, current_user.email)
    if existing:
...
async def update_alert(
    ...
    if alert is None or alert.email != current_user.email:
        raise HTTPException(status_code=404, detail="Alert not found")
    updated = await repo.update(aid, target_price=body.target_price, active=body.active)
